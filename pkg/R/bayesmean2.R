...
.ws8 = proto(
  ...
  create = function(.,h,...) {
    ...
    if("Inférence bayésienne\nsur une moyenne\n(informatif)" %in% names(.ws$nb)) return()
   ...
   .$sdFixed = gcheckbox("Connu",checked=TRUE,handler=.$onCheck)
  ...
  updatePlot = function(.,h,...) {
    ...
    if(svalue(.$sdFixed)) {
      ...
      if(!is.null(m0) && !is.null(prior.prob)) {
        ...
        BF = dnorm(m0,post.mean,post.sd)/dnorm(m0,prior.mean,prior.sd)
    ...
    # Case 2: Variance unknown
      ...
      if(!is.null(m0) && !is.null(prior.prob)) {
        ...
        # Default two-sided test
        BF = dt(sqrt(post.n)*(m0-post.mean)/scale,nu.n) / dnorm(m0,prior.mean,prior.sd/sqrt(prior.n))
        post.prob = prior.prob * BF/(prior.prob * BF + 1 - prior.prob)
        
        if(op != "H : mu =") {
          priorH = pt(sqrt(prior.n)*(m0-prior.mean)/prior.sd,prior.n)
          priorA = 1 - priorH
          prior.odds = priorH/priorA
